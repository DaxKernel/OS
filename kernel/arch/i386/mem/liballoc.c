...
static unsigned int l_pageSize = frame_size; ///< The size of an individual page. Set up in liballoc_init.
...
void *malloc(size_t req_size)
{
	...
	if (ALIGNMENT > 1)
	{
		...
		We can use gcc bultin.
		 - R Midhun Suresh
		size += ALIGNMENT + ALIGN_INFO;
		*/
		if (__builtin_uaddl_overflow(size, ALIGNMENT + ALIGN_INFO, &size))
		{
			return NULL;
		}
	}
